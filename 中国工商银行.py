...
def access1(name):
    ...
    sql = "select * from use where username = %s"
    param = [name]
    date=select(sql,param)
    print(type(date))
    if len(date) !=0:
        money = int(input('请输入存储金额：'))
        sql = "update use set money = money + %s where username = %s"
        param = [money,name]
        update(sql,param)
        return True
    else:return False
...
def shift1(off_name,on_name,off_passwd,off_money):
    ...
    sql = "select * from user where username = %s or username = %s order by username"
    ...
    if len(date) >= 2:
        if off_passwd == date1:
            if off_money <=date2:
                ...
                update(sql,param)
                sql1 = "update user set money = money + %s where username = %s"
